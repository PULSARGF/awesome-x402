...
- [Gate402](https://gate402.app) - x402 reverse-proxy payment gateway for M2M agent APIs. Unified `/v1/*` storefront with **8 pay-per-call endpoints, all listed in the CDP x402 Bazaar**. Finance/crypto data for trading agents: on-chain wallet/token intel ($0.01), DEX price/liquidity + cross-DEX arbitrage ($0.01), market news + bull/bear sentiment ($0.02), SEC EDGAR filings ($0.02) — plus web→Markdown scraping ($0.002, + $0.05 stealth tier), token compression (~40% fewer LLM tokens, $0.005/10k), and a semantic vector cache ($0.001 hit / $0.003 miss). Multi-network USDC (Base/Polygon/Arbitrum/Solana) via the Coinbase CDP facilitator, x402 **or** a non-crypto API-key/Stripe rail, no signup. Also an MCP server — `npx -y gate402-mcp` (11 tools, free-credit tier). ([Discovery](https://gate402.app/.well-known/x402.json)) ([Catalog](https://gate402.app/v1)) ([npm](https://www.npmjs.com/package/gate402-mcp)) ([MCP Registry](https://registry.modelcontextprotocol.io/v0/servers?search=io.github.pgalyen1987))
