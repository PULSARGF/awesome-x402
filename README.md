...
- [DDG Agent-Payable Services](https://agents.daedalusdevelopmentgroup.com) — Machine-readable security and commerce primitives for AI agents: x402 checkout-conformance audits, MCP/tool security preflight, spend-authorization contracts, browser-agent completion proofs, and bounded artifact/model orders. Identity-first (403→402→receipt), 5 settlement networks (Base, Polygon, Arbitrum, World Chain, Solana USDC). $0.01–$5 USDC per call. MCP server + PyPI package ([ddg-agent-services-mcp](https://pypi.org/project/ddg-agent-services-mcp/)). ([OpenAPI](https://agents.daedalusdevelopmentgroup.com/openapi.json) | [llms.txt](https://agents.daedalusdevelopmentgroup.com/llms.txt) | [Status](https://agents.daedalusdevelopmentgroup.com/.well-known/ddg-agent-status.json) | [GitHub](https://github.com/daedalusdevelopmentgroup/ddg-agent-payable-services))
