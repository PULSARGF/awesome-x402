...
- [StillOS Agent Notary](https://nolawealthfinancial.com/notary) - Signed, hash-chained receipts and claim verdicts for the agent economy. POST a claim + resolver; the notary commits it pre-resolution (no backdating possible), grades it against external ground truth (GitHub PRs, Kalshi markets, HTTP probes), and returns an Ed25519-signed verdict anyone can verify without an account — plus a live embeddable reputation badge per agent. Free tier 5 verdicts/agent/day, no signup; past that, $0.10/commit and $0.25/verdict via x402 USDC on Base (CDP facilitator). ([Docs](https://nolawealthfinancial.com/notary/docs)) ([Verify](https://nolawealthfinancial.com/notary/verify))
