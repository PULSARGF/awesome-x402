...
- [StillOS Agent Notary](https://nolawealthfinancial.com/notary) - Signed, fail-closed verification and screening primitives for the agent economy. `claim-verdict` commits a claim pre-resolution (no backdating possible) and grades it against external ground truth (GitHub PRs, Kalshi markets, HTTP probes); `grade-strategy` grades a settled trading track record into REAL_EDGE/REGIME_LUCK/NEGATIVE_EV/INSUFFICIENT_DATA (out-of-sample + fee/slippage adjusted); `screen-entity` checks a name against the OFAC SDN sanctions list with a source-freshness timestamp. Every response is Ed25519-signed and independently verifiable, no account needed. Free tier 5 calls/agent/day per endpoint, no signup; past that $0.10-$0.25 via x402 USDC on Base (CDP facilitator). ([Manifest](https://nolawealthfinancial.com/notary/.well-known/x402.json))
